<template>
  <Modal :show="show" role="alertdialog" id="o1-nmh-confirm-rename-modal" @close-via-escape="$emit('close')">
    <div class="o1-bg-white dark:o1-bg-gray-800 o1-rounded-lg o1-shadow-lg o1-overflow-hidden" style="width: 460px">
      <ModalHeader v-text="'Kollektion erstellen'" />

      <ModalContent>
        <input v-model="collectionName" type="text" class="form-control form-input form-input-bordered w-full mt-4"
...
export default {
  ...
  methods: {
    async handleRename() {
      ...
      const result = await this.createCollection(this.collectionName);
